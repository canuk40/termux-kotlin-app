...
object KeyHandler {
    ...
    @JvmStatic
    fun getCodeForKitty(keyCode: Int, keyMod: Int, codePoint: Int): String? {
        val kittyMod = keyModToKittyMod(keyMod)
        
        // Special handling for function keys and navigation keys in Kitty protocol
        return when (keyCode) {
            KEYCODE_ESCAPE, KEYCODE_BACK -> if (kittyMod > 0) "\u001b[27;${kittyMod+1}u" else null
            KEYCODE_ENTER, KEYCODE_NUMPAD_ENTER -> if (kittyMod > 0) "\u001b[13;${kittyMod+1}u" else null
            KEYCODE_TAB -> if (kittyMod > 0) "\u001b[9;${kittyMod+1}u" else null
            KEYCODE_DEL -> if (kittyMod > 0) "\u001b[127;${kittyMod+1}u" else null
            
            // Navigation keys with optional modifiers
            KEYCODE_DPAD_UP -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}A" else null
            KEYCODE_DPAD_DOWN -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}B" else null
            KEYCODE_DPAD_RIGHT -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}C" else null
            KEYCODE_DPAD_LEFT -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}D" else null
            KEYCODE_MOVE_HOME -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}H" else null
            KEYCODE_MOVE_END -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}F" else null
            
            // Function keys
            KEYCODE_F1 -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}P" else null
            KEYCODE_F2 -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}Q" else null
            KEYCODE_F3 -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}R" else null
            KEYCODE_F4 -> if (kittyMod > 0) "\u001b[1;${kittyMod+1}S" else null
            KEYCODE_F5 -> if (kittyMod > 0) "\u001b[15;${kittyMod+1}~" else null
            KEYCODE_F6 -> if (kittyMod > 0) "\u001b[17;${kittyMod+1}~" else null
            KEYCODE_F7 -> if (kittyMod > 0) "\u001b[18;${kittyMod+1}~" else null
            KEYCODE_F8 -> if (kittyMod > 0) "\u001b[19;${kittyMod+1}~" else null
            KEYCODE_F9 -> if (kittyMod > 0) "\u001b[20;${kittyMod+1}~" else null
            KEYCODE_F10 -> if (kittyMod > 0) "\u001b[21;${kittyMod+1}~" else null
            KEYCODE_F11 -> if (kittyMod > 0) "\u001b[23;${kittyMod+1}~" else null
            KEYCODE_F12 -> if (kittyMod > 0) "\u001b[24;${kittyMod+1}~" else null
            
            KEYCODE_INSERT -> if (kittyMod > 0) "\u001b[2;${kittyMod+1}~" else null
            KEYCODE_FORWARD_DEL -> if (kittyMod > 0) "\u001b[3;${kittyMod+1}~" else null
            KEYCODE_PAGE_UP -> if (kittyMod > 0) "\u001b[5;${kittyMod+1}~" else null
            KEYCODE_PAGE_DOWN -> if (kittyMod > 0) "\u001b[6;${kittyMod+1}~" else null
            
            // For printable characters, use CSI u format
            else -> {
                if (codePoint > 0 && kittyMod > 0) {
                    "\u001b[${codePoint};${kittyMod+1}u"
                } else {
                    null
                }
            }
    ...
    private fun keyModToKittyMod(keyMod: Int): Int {
        var mod = 0
