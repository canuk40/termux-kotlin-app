...
class TerminalView(context: Context, attributes: AttributeSet?) : View(context, attributes) {
    ...
    fun inputCodePoint(eventSource: Int, codePoint: Int, controlDownFromEvent: Boolean, leftAltDownFromEvent: Boolean) {
        ...
        // Check if Kitty keyboard mode is enabled and we have modifiers
        if (mEmulator?.isKittyKeyboardMode() == true && (controlDown || altDown)) {
            var keyMod = 0
            if (controlDown) keyMod = keyMod or KeyHandler.KEYMOD_CTRL
            if (altDown) keyMod = keyMod or KeyHandler.KEYMOD_ALT
            
            // For printable characters with modifiers in Kitty mode, use CSI u
            val kittyCode = KeyHandler.getCodeForKitty(0, keyMod, cp)
            if (kittyCode != null) {
    ...
    fun handleKeyCode(keyCode: Int, keyMod: Int): Boolean {
        ...
        // Check if Kitty keyboard mode is enabled
        val code = if (term!!.isKittyKeyboardMode()) {
            KeyHandler.getCodeForKitty(keyCode, keyMod, 0) ?: KeyHandler.getCode(keyCode, keyMod, term.isCursorKeysApplicationMode(), term.isKeypadApplicationMode())
        } else {
